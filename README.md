...
`SUPABASE_SERVICE_ROLE_KEY`、`ANTHROPIC_API_KEY`、`OPENROUTER_API_KEY` 和 `HISTORICAL_RESEARCH_API_KEY` 只应配置在服务端环境变量中，不要写进前端代码或提交到 Git。项目的 `.gitignore` 会忽略 `.env.local`。
...
3. 如果同时配置了 OpenRouter，后端会优先使用 OpenRouter。

## OpenRouter 配置

1. 将 OpenRouter API Key 填入 `OPENROUTER_API_KEY`。
2. `OPENROUTER_MODEL` 默认使用 `anthropic/claude-sonnet-4`，可按需要替换。
3. 对无法由基础规则直接判断的问题，必须配置 OpenRouter 或 Anthropic key；否则提交问题会返回明确的 AI 未配置错误。
